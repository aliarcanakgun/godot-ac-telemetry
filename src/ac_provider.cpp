...
void ACProvider::apply_math_conversions_in_place(LapDataChannels& lap) {
...
Dictionary ACProvider::_lap_to_dict(const LapDataChannels& c) {
    Dictionary d;
...
Dictionary ACProvider::get_live_snapshot() {
    ...
    LapDataChannels lap_copy = sessions_data.back();
...
Dictionary ACProvider::get_lap_data(int lap_index) {
    ...
    LapDataChannels lap_copy = loaded_session_data[lap_index];
    apply_math_conversions_in_place(lap_copy);
    return _lap_to_dict(lap_copy);
}
...
String ACProvider::_open_session_file(const String& file_path, std::ifstream& infile, SPageStatic& out_static, double& out_sample_interval, double& out_samples_per_meter, uint64_t& out_lap_count, std::vector<uint64_t>& out_lap_offsets) {
...
Dictionary ACProvider::_calculate_session_metadata(const SPageStatic& stat, uint64_t count, const std::vector<LapDataChannels>& laps) {
    Dictionary meta;
...
Dictionary ACProvider::_static_to_dict(const SPageStatic &s) {
    Dictionary dict;
    // Original raw AC keys
    ...
    dict["carModel"] = wchar_to_gdstring(s.carModel, 33);
    dict["track"] = wchar_to_gdstring(s.track, 33);
    ...
    dict["sectorCount"] = s.sectorCount;
    dict["maxTorque"] = s.maxTorque;
    dict["maxPower"] = s.maxPower;
    dict["maxRpm"] = s.maxRpm;
    ...
    dict["trackSPlineLength"] = s.trackSPlineLength;
    dict["trackConfiguration"] = wchar_to_gdstring(s.trackConfiguration, 33);
    ...
    dict["track_name"] = wchar_to_gdstring(s.track, 33);
